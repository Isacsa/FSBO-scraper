/**
 * Parse e normaliza dados brutos extraídos do OLX
...
/**
 * Parse de datas usando o módulo específico do OLX
 */
const { parseOLXDate } = require('./dateParser');
...
function parseDates(dates) {
  ...
  const published_date = dates.published ? parseOLXDate(dates.published, now) : null;
  const updated_date = dates.updated ? parseOLXDate(dates.updated, now) : null;
...
function parseRawData(raw) {
  console.log('[OLX Parse] 🔍 Iniciando parsing de dados...');
  ...
  console.log('[OLX Parse] ✅ Parsing concluído');
