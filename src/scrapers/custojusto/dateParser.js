/**
 * Parser de datas específico para CustoJusto
 * Processa formatos de data que o CustoJusto usa e retorna ISO UTC
 */
...
function parseAbsoluteDate(text, now = new Date()) {
  ...
  // Formato específico do CustoJusto: "DD MMM" (ex: "27 Nov")
  ...
  // Padrão "DD MMM" (ex: "27 Nov", "5 Jan")
  const custoJustoDateMatch = text.match(/(\d{1,2})\s+([a-z]{3,4})\.?/i);
  if (custoJustoDateMatch) {
    const [, day, monthAbbr] = custoJustoDateMatch;
    const month = monthAbbreviations[monthAbbr.toLowerCase()];
    if (month !== undefined) {
      const dayNum = parseInt(day, 10);
      let finalYear = currentYear;
      
      // Criar data com ano atual primeiro
      const date = new Date(finalYear, month, dayNum);
      
      // Se a data for futura (ex: 27 Nov quando estamos em Janeiro de 2025),
      // usar ano anterior (2024)
      // Também verificar se o mês é anterior ao mês atual (ex: Nov quando estamos em Jan)
      if (date > now || month < currentMonth) {
        finalYear = currentYear - 1;
        date.setFullYear(finalYear);
      }
      
      // Se ainda for futura (caso raro), usar ano anterior
      if (date > now) {
        finalYear = currentYear - 1;
        date.setFullYear(finalYear);
      }
...
/**
 * Função principal de parsing de datas do CustoJusto
 ...
 */
function parseCustoJustoDate(dateText, now = new Date()) {
  ...
  console.warn(`[CustoJusto DateParser] ⚠️  Não foi possível parsear a data: "${dateText}"`);
  return null;
}
